#!/usr/bin/env bash
# Requires: nmcli, rofi
# Note: SSIDs containing ':' may not parse correctly (nmcli terse format limitation)
...
lines+=("󰁪  Refresh")
etypes+=("refresh"); essids+=(""); econns+=(""); esecs+=("")
...
etypes+=("toggle"); essids+=(""); econns+=(""); esecs+=("")
...
if wifi_enabled; then
    ...
    if [ -n "$active_ssid" ]; then
        ...
        etypes+=("connected"); essids+=("$active_ssid"); econns+=(""); esecs+=("")
        seen["$active_ssid"]=1
    fi
    ...
    while IFS=: read -r _active ssid signal security; do
        ...
        if [ -n "${saved_conns[$ssid]}" ]; then
            ...
            etypes+=("saved"); essids+=("$ssid"); econns+=("$ssid"); esecs+=("")
        else
            lines+=("$icon  $ssid$lock")
            etypes+=("new"); essids+=("$ssid"); econns+=(""); esecs+=("$security")
        fi
